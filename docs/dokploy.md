...
Runbook end-to-end para deployar el monorepo (`apps/api`, `apps/web`, `apps/admin`) más infraestructura (Postgres, Valkey, MinIO, Plausible) en un VPS con Dokploy.
...
### SSL/TLS en Cloudflare (si usas proxy en el futuro)

Si algún día activas el proxy de Cloudflare:
- SSL/TLS → Overview → **Full (strict)**.
- Asegúrate de tener un Origin Certificate o cert válido en el servidor.
...
El Dockerfile actual usa **dos stages**:
...
DATABASE_URL=postgresql://jcsoftdev:<password>@postgres:5432/jcsoftdev
...
Click **Deploy**. El container debe quedar `running`.

### 4.7 Migraciones de base de datos (primera vez y tras schema changes)

Las migraciones **no corren automáticamente** en el deploy. Córrelas manualmente la primera vez y cada vez que haya un schema change:

1. En Dokploy → api → General → **Open Terminal**
2. En el dropdown, selecciona el container con estado **running**
3. Selecciona **/bin/sh** (no bash — la imagen es Alpine)
4. Ejecuta:
...
> El script de migración es idempotente — se puede correr múltiples veces sin problema.
...
Abre `https://admin.jcsoftdev.com` en browser → debe cargar el panel de login con magic link.

---

## 7. Validar certificados SSL en Dokploy

Por cada servicio: **Domains** tab → botón **Validate** junto al cert.

Debe mostrar **Valid** (verde). Si muestra error:
- Verifica que el DNS record esté en **DNS only** (no Proxied).
- Verifica que el puerto 80 y 443 estén accesibles desde internet.
- Espera 2-5 min y vuelve a intentar.

---

## 8. Workflow de cambios después del setup inicial

### 8.1 Schema change (nueva migración)
...
Dokploy redeploya automáticamente (Autodeploy ON). Luego corre las migraciones manualmente:

1. api → General → **Open Terminal** → container running → `/bin/sh`
2. `cd /repo/packages/db && bun run src/migrate.ts`

### 8.2 Cambio en api/web/admin

```bash
git push origin main
```

Con **Autodeploy ON** en Dokploy → deploy automático en todos los servicios.

### 8.3 Rollback

En Dokploy → service → **Deployments** → click en deploy anterior → **Redeploy**.

Para rollback de migración → escribe migración inversa nueva (drizzle no soporta down automático).
...
Configura `rclone config` una vez con credenciales del bucket externo (Backblaze B2, S3, Cloudflare R2).

---

## 10. Servicios secundarios (Plausible + MinIO console)
...
| Container exits immediately, exit 255, sin logs (continúa tras fix de Zod) | Binario compilado con `bun build --compile` en Alpine no corre en imagen glibc | Usar `bun run src/index.ts` en `oven/bun:1-alpine`, sin compilar a binario |
| api 502 Bad Gateway | Container crasheó en startup | Logs → fix env var faltante o DB connection |
| `PostgresError: relation "X" does not exist` | Migraciones no corridas | Open Terminal → container running → `/bin/sh` → `cd /repo/packages/db && bun run src/migrate.ts` |
| Let's Encrypt failed: HTTP-01 challenge | DNS está Proxied en Cloudflare | Flip a DNS only, redeploy, esperar cert |
| web en blanco / `PUBLIC_API_URL must be set` | Variables `PUBLIC_*` no disponibles en build time | Agregar `ARG`/`ENV` antes de `pnpm run build` en el Dockerfile |
| web SSR falla, `Cannot find module` | Stage 2 solo copia `dist/`, faltan `node_modules` | Stage 2 debe copiar `/repo` completo del stage de build |
| Terminal del container muestra `bash not found` | Imagen es Alpine, no tiene bash | Usar `/bin/sh` en lugar de `Bash` en el selector del Docker Terminal |
...
| Plausible login error | `BASE_URL` no coincide con dominio real | Cambiar `DOMAIN` env, redeploy compose |
...
## 12. Cheat sheet de URLs y servicios
...
## 13. Resumen del flujo "release nuevo"

```
1. local: git push origin main
2. Dokploy autodeploy detecta commit
3. Build: Dockerfile multi-stage
4. Deploy: container nuevo, swap atómico
5. Si hay schema changes: Open Terminal → bun run src/migrate.ts
